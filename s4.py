...
# Temp/buffer globals.
TEMP_FILES = set()
BUFFER_KEYS = ["dirs", "files", "buckets"]
...
class Options:
  ...
  def __init__(self, opt=None):
    ...
    if opt:
      ...
      '''Assign Argparse NameSpace to Class.dict'''
      [setattr(self, key, opt[key]) for key in opt]
  ...
  def join(self):
    ...
    try:
      while self.unfinished_tasks:
        ...
        if self.exc_info:
          fail(INDENT + '[Thread Failure] ', exc_info = self.exc_info)
...
class ThreadPool(object):
    ...
    def run(self):
      ...
      while True:
        ...
        try:
          ...
          self.pool.tasks.terminate(e)
          fail(INDENT + '[Runtime Failure] ', exc_info = e)
        ...
        #   fail('[S3ResponseError] %s: %s' % (e.error_code, e.error_message))
          ...
          self.pool.tasks.terminate(e)
          fail(INDENT + '[OSError] %d: %s' % (e.errno, e.strerror))
        except Exception, e:
          # XXX Should we retry on all unknown exceptions?
          if retry >= self.opt.retry:
            self.pool.tasks.terminate(e)
            fail(INDENT + '[Runtime Exception] ', exc_info = e, stacktrace = True)
          else:
  ...
  def processed(self):
    ...
    if qsize > 0:
      progress(INDENT + '[%d task(s) completed, %d remaining, %d thread(s)]', self.processed_tasks, qsize, len(self.workers))
    else:
      progress(INDENT + '[%d task(s) completed, %d thread(s)]', self.processed_tasks, len(self.workers))
...
class S3Handler(object):
  ...
  def list_buckets(self):
    ...
    for bucket in self.s3.get_all_buckets():
      self.buffer["dirs"].append(S3URL.combine('s3', bucket.name, ''))
      self.buffer["buckets"].append({
  ...
  def s3walk(self, basedir, show_dir = None):
    ...
    self.sorted = sorted(result, compare)
    for x in self.sorted:
      if x['is_dir']: self.buffer["dirs"].append(x['name']) 
      else: self.buffer["files"].append(x['name'])
    return self.sorted
  ...
  def source_expand(self, source):
    ...
    if (len(result) == 0) and (not self.opt.ignore_empty_source):
      fail(INDENT + "[Runtime Failure] Source doesn't exist.")
  ...
  def put_files(self, source, target):
    ...
    '''
      ...
      Also, it works for recursive mode which copies all files and keeps the
...
class ThreadUtil(S3Handler, ThreadPool.Worker):
  ...
  def upload(self, mpi, source, target, pos = 0, chunk = 0, part = 0):
    ...
    if not mpi:
      ...
      # optional checks
      if self.opt.dry_run:
        message(INDENT + '%s => %s', source, target)
        return
      elif self.opt.sync_check and self.sync_check(source, key):
        message(INDENT + '%s => %s (synced)', source, target)
        return
  ...
  def delete(self, source):
    ...
    if not self.opt.dry_run:
      key.delete()
    message(INDENT + 'Delete %s', source)
...
class CommandHandler(object):
  ...
  def run(self, opt, args):
    ...
    if len(args) == 0:
      raise InvalidArgument(INDENT + 'No command provided')
    ...
    if CommandHandler.__dict__.has_key(cmd + '_handler'):
      ...
      except InvalidArgument, e: fail(INDENT + '[Invalid Argument] ', exc_info = e)
      except Failure, e: fail(INDENT + '[Runtime Failure] ', exc_info = e)
      except boto.exception.S3ResponseError, e: fail(INDENT + '[S3ResponseError] %s: %s' % (e.error_code, e.error_message))
      except Exception, e: fail(INDENT + '[Runtime Exception] ', exc_info = e, stacktrace = True)

    else:
      raise InvalidArgument(INDENT + 'Unknown command %s' % cmd)
  ...
  def ls_handler(self, args):
    ...
    if len(args) == 1:
      args.append(self.pathprefix)
      if self.pathprefix == "/" :
        s3handler = self.s3handler()
        bucket_list = s3handler.list_buckets()
        self.pretty_print(bucket_list["buckets"])
        self.buffer = s3handler.buffer
        return        

    self.validate('cmd|s3', args)
    s3handler = self.s3handler()
    s3walk = s3handler.s3walk(args[1])
    self.buffer = s3handler.buffer
    self.pretty_print(s3walk)
  ...
  def put_handler(self, args):
    ...
    args[-1] = self.pathprefix + args[1:-1][0] if args[-1] == "." else args[-1]
    self.validate('|'.join(['cmd'] + ['local'] * (len(args) - 2) + ['s3']), args)

    source = args[1:-1]; target = args[-1]
  ...
  def cd_handler(self, args):
    ...
    assert args[-1].startswith("s3://") and args[-1].endswith("/") or args[-1] == "/"
    self.pathprefix = args[-1]
    return
